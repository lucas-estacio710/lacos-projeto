...
export function PixInterModal({
  ...
  useEffect(() => {
    ...
    // ✅ CORREÇÃO RADICAL: Só processar dados se ainda não temos dados organizados
    if (dayGroups.length > 0) {
      console.log('🔒 Dados já organizados, pulando re-processamento para preservar seleções');
      return;
    }

    console.log('🔧 Organizando dados PIX Inter por dia...');

    // 1. Filtrar apenas PIX Inter com categoria COMPLEXA E não reconciliadas
    const pixTransactions = complexTransactions.filter(t => {
      ...
      // ✅ CORREÇÃO: Apenas categoria COMPLEXA E não reconciliadas
      ...
      console.log(`🔍 Transação ${t.id}:`, {
        ...
        isComplexCategory,
        isNotReconciled,
        shouldInclude: isPixInter && isComplexCategory && isNotReconciled
      });
      
      return isPixInter && isComplexCategory && isNotReconciled;
    });

    // 2. Filtrar apenas entradas PIX da planilha
  ...
  }, [isOpen]); // ✅ CORREÇÃO CRÍTICA: Remover complexTransactions e planilhaEntries das dependencies
  ...
  // Gerar classificação baseada na entrada da planilha
  const generateClassificationFromEntry = (entry: EntradaFinanceira): {
    ...
    return {
      ...
      subtipo,
      descricao: `Receita Nova ${isPlano ? 'Plano' : 'Catálogo'} ${isIndividual ? 'Individual' : 'Coletiva'} - Contrato ${entry.id_contrato}`
    };
  };
